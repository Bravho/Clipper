...
**Model:** First request generates free; payment (50 credits) is required only at the final step to download without watermark. Second request onward: 50 credits charged at submission, download unlocked immediately.
...
| Paid submissions (2nd+) | same | Requires + deducts `REQUEST_COST_CREDITS` (50) at submission |
| Pay-to-download | `ClipRequestService.unlockDownload()` | Charges 50, sets `downloadUnlocked: true`, idempotent |
...
| Pricing config | `config/credits.ts` | 100 full / 50 launch price via `LAUNCH_DISCOUNT_ACTIVE`; top-up bundles 50–1000 |
...
1. **Dashboard warning** (`dashboard/page.tsx`): `canAfford = balance >= 50` with no trial awareness → new user is told they can't submit. The "+ คำขอใหม่" CTA also reads as disabled.
...
2. Dashboard shows a **trial banner** instead of the insufficient-credits warning: "คลิปแรกฟรี — สร้างได้เลย จ่ายเมื่อพอใจตอนดาวน์โหลด (50 เครดิต)".
3. New-request form submits with 0 credits; form shows cost as "ฟรี (จ่าย 50 เครดิตตอนดาวน์โหลด)".
4. Pipeline runs normally. All previews the user sees are the watermarked siblings.
5. At final approval/complete, `UnlockDownloadPanel` shows: pay ฿50 (top-up → auto-unlock via `unlockRequest` param) → `downloadUnlocked` → clean masters downloadable in all ratios.
6. Never pays → keeps watermarked preview only; clean master retained but locked.

### Flow B — Second request onward
1. Dashboard/form show the real gate: balance < 50 → warning + link to top-up (not "contact support").
2. Submission checks + deducts 50 immediately; `downloadUnlocked: true` from the start.
...
**D4. Price at unlock = price at submission-time list.** `unlockDownload()` reads `REQUEST_COST_CREDITS` at unlock time. If `LAUNCH_DISCOUNT_ACTIVE` is toggled off while a trial is in flight, the user would be quoted 100 after being promised 50. **Recommendation:** snapshot the unlock price onto the request at submission (`unlockPriceCredits: 50`) and charge that.
...
2. `dashboard/page.tsx` → when `trialAvailable`, replace the yellow warning with a green/blue trial banner; keep "+ คำขอใหม่" fully enabled. Insufficient-credits warning only when `!trialAvailable && balance < 50`, and link it to `/dashboard/credits` (top-up) instead of "contact support".
3. `requests/new/page.tsx` → compute `trialAvailable`, pass through `PackageSelector` → `NewRequestForm`.
4. `NewRequestForm` → new prop `trialAvailable`; when true: `insufficientCredits` is ignored, cost display becomes "ฟรี — จ่าย 50 เครดิตเมื่อดาวน์โหลดแบบไม่มีลายน้ำ", submit enabled.
5. Pricing card copy: "คลิปแรกสร้างฟรี ดาวน์โหลดไฟล์จริง 50 เครดิต · คำขอถัดไป 50 เครดิต/รายการ".
...
### Phase 3 — Tests
10. `tests/services/ClipRequestService.test.ts` (fresh Mock repos pattern): first submit free at 0 balance; second submit blocked at <50 / charged at ≥50; unlock idempotent; unlock blocked before finals exist; unlock uses snapshotted price; rejected trial restores eligibility; refund path.
...
Top-up (PromptPay/card, 1cr = ฿1)  ──►  wallet balance
        ...
        submit: charge 0                  │   submit: charge 50  ──► RequestCharge(-50)
        generate + watermark preview      │   downloadUnlocked = true
        unlock: charge 50 ────────────────┘
          └► RequestCharge(-50), downloadUnlocked = true
        pipeline fails after payment ──► RequestRefund(+50)
```
