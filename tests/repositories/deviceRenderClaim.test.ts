...
describe("device render claims", () => {
  ...
  it("only the request owner can claim a queued overlay", async () => {
    ...
    expect(await repo.claimForDevice(task.id, "other", "device:other:1")).toBeNull();
    expect(await repo.claimForDevice(task.id, "owner", "device:owner:1")).toMatchObject({ state: "claimed" });
    expect(await repo.claimForDevice(task.id, "owner", "device:owner:2")).toBeNull();
  });

  it("an old device cannot complete or release a worker-reclaimed task", async () => {
    const repo = setup();
    const task = await repo.enqueue({ jobId: "job", requestId: "request", requesterId: "owner", step: RenderStep.OverlayComposition });
    await repo.claimForDevice(task.id, "owner", "device:owner:1");
