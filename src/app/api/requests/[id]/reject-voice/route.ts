...
export async function POST(
  ...
  try {
    const updated = await videoGenerationService.rejectVoiceConversionByRequester(jobId, session.user.id);
    return NextResponse.json({ currentStep: updated.currentStep });
  } catch (err) {
    const message = err instanceof Error ? err.message : "Failed to reject voice.";
