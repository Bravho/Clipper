...
export async function POST(
  ...
  try {
    const updated = await videoGenerationService.regenerateAnimationByRequester(
      jobId,
      session.user.id
    );
    return NextResponse.json({ currentStep: updated.currentStep });
  } catch (err) {
    const message = err instanceof Error ? err.message : "Failed to regenerate animation.";
