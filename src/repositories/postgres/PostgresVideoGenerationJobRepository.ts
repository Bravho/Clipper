...
function rowToJob(row: Record<string, unknown>): VideoGenerationJob {
  return {
    ...
    finalExport_travy_assetId: (row.final_export_travy_asset_id as string) ?? null,
    ...
    travyVideoStatus:
      (row.travy_video_status as "idle" | "generating" | "ready" | "failed") ?? "idle",
    travyVideoError: (row.travy_video_error as string) ?? null,
...
const JOB_UPDATE_COLS: Record<string, string> = {
  ...
  "finalExport_travy_assetId": "final_export_travy_asset_id",
  ...
  travyVideoStatus: "travy_video_status",
  travyVideoError: "travy_video_error",
  ...
  async create(
    ...
    const { rows } = await this.db.query(
      `INSERT INTO video_generation_jobs (
         ...
         final_export_1_1_asset_id, final_export_4_5_asset_id, final_export_travy_asset_id,
         subtitle_languages,
      ...
      [
        ...
        input.finalExport_travy_assetId ?? null,
